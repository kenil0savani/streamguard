...
PG_DSN = os.getenv(
    "PG_DSN", "dbname=streamguard user=streamguard password=streamguard host=localhost port=5432"
)
...
def get_conn(retries: int = 15):
    for attempt in range(retries):
        try:
            ...
            print(f"Postgres not ready yet ({exc}); retrying in 3s... [{attempt + 1}/{retries}]")
            time.sleep(3)
...
def main() -> None:
    ...
    for message in consumer:
        ...
        cur.execute(
            ...
            VALUES (%s, %s, %s, %s)
            """,
            (event["sensor_id"], event["timestamp"], result["anomaly_score"], result["is_anomaly"]),
        )
